...
from Food_Order_App.models import Profile,Product
    ...
    if request.POST:
        ...
        if request.user.is_authenticated:
            ...
            for product in cart_products:
                ...
                for key,value in quantities.items():
                    if int(key) == product.id:
                        create_order_item = OrderItem(order_id=order_id, product_id=product.id, user=user, quantity=value, price=price)
            ...
            for product in cart_products:
                ...
                for key,value in quantities.items():
                    if int(key) == product.id:
                        create_order_item = OrderItem(order_id=order_id, product_id=product.id, quantity=value, price=price)
                        create_order_item.save()            
...
def billing_info(request):
    if request.POST:
        ...
        paypal_form = PayPalPaymentsForm(initial = paypal_dictionary)
